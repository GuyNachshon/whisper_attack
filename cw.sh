...
LR=0.01
DEVICE=cuda
python run_attack.py attack_configs/whisper/cw.yaml --root=$RSROOT --device=$DEVICE --model_label=tiny --nb_iter=$NBITER --load_audio=$LOAD --seed=$SEED --attack_name=$NAME --eps=$EPS --const=$CST --max_decr=$MAXDECR --lr=$LR --confidence=$CONF --decrease_factor_eps=$DECRFACTOR
